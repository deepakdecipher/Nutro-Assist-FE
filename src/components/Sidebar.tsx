...
import { clearAuth, getSavedAdminRoles } from "@/lib/api";
...
export function Sidebar({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const isAdmin = getSavedAdminRoles().some((r) => ["ADMIN", "SUPER_ADMIN"].includes(r));
  ...
  return (
    <>
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 flex flex-col w-64 transition-transform duration-300 lg:translate-x-0 lg:static lg:z-auto ${open ? "translate-x-0" : "-translate-x-full"}`}
        ...
        {isAdmin && (
          <div className="px-3 pb-2 border-t pt-3" style={{ borderColor: "rgba(139,92,246,0.08)" }}>
            <Link href="/admin/dashboard" onClick={onClose}
              className="flex items-center gap-3 w-full px-3 py-2.5 rounded-xl text-sm text-violet-600 hover:bg-violet-50 hover:text-violet-700 transition-all duration-200 group">
